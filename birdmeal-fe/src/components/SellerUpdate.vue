<template>
    <v-dialog v-model="dialog" persistent>
      <template v-slot:activator="{ props }">
        ...
        v-bind="props" @click="getSellerInfo"
        >수정하기</v-btn
      >
      </template>
  ...
  <v-container>
    ...
    <v-row style="height: 100px" align="center">
        ...
        <v-col cols="4" offset-md="1"><v-file-input
            label= "판매자 이미지를 넣어주세요"
            id="sellerUpdateImg"
    ...
    <v-row style="height: 100px" align="center">
        ...
        <v-col cols="7" offset-md="1"><v-text-field v-model="sellerInfo.sellerNickname" solo color="green"></v-text-field
          ></v-col>
      </v-row>
      <v-row style="height: 100px" align="center">
        <v-col cols="2" offset-md="1">판매자 전화번호</v-col>
        <v-col cols="7" offset-md="1"><v-text-field v-model="sellerInfo.sellerTel" solo color="green"></v-text-field
          ></v-col>
      </v-row>
      <v-row style="height: 100px" align="center">
        <v-col cols="2" offset-md="1">판매자 주소</v-col>
        <v-col cols="7" offset-md="1"><v-text-field v-model="sellerInfo.sellerAddress" solo color="green"></v-text-field
          ></v-col>
      </v-row>
      <v-row style="height: 100px" align="center">
        <v-col cols="2" offset-md="1">판매자 정보</v-col>
        <v-col cols="7" offset-md="1"><v-text-field v-model="sellerInfo.sellerInfo" solo color="green"></v-text-field
          ></v-col>
      </v-row>
      <v-row v-if="!sellerInfo.sellerNickname || !sellerInfo.sellerTel || !sellerInfo.sellerAddress || !sellerInfo.sellerInfo">
          <v-alert
...
const sellerInfo = ref({});
  ...
  function getSellerInfo() {
    ...
    function save() {
        if (!sellerInfo.value.sellerNickname || !sellerInfo.value.sellerEmail || !sellerInfo.value.sellerTel || !sellerInfo.value.sellerAddress || !sellerInfo.value.sellerInfo) {
        return;
    }
        let delivery = {
